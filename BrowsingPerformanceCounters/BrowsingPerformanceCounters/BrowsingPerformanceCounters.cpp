// BrowsingPerformanceCounters.cpp : 定义控制台应用程序的入口点。
//
...
void wmain(void)
{
    ...
    WCHAR CounterPathBuffer[PDH_MAX_COUNTER_PATH] =
        //L"\\Network Interface(Realtek 8821CE Wireless LAN 802.11ac PCI-E NIC)\\Bytes Received/sec";
        L"\\Process(chrome.exe)\\Bytes Received/sec";
    ...
    while (!_kbhit())
    {
        ...
        WCHAR STR[200];
        wsprintf(STR, L"%.20g\/s", DisplayValue.doubleValue);
        wprintf(L"%.2g Kbps", DisplayValue.doubleValue / 1024 * 8);
    }
